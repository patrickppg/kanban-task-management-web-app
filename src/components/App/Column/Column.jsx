...
function Column({ column, onViewTask }) {
  ...
  // When a task is added, make the first task tabbable.
  useEffect(() => {
    if (column) {
  ...
  return (
    <div
      ...
      className="column"
      role="grid"
      aria-label={`Tasks ${column.name}`}
      aria-description={`${column.tasks.length} tasks`}>
      <div
        aria-hidden="true" 
        className="text-column-name">
      ...
      {column.tasks.map(task => (
        <div key={task.id} role="row">
          <div role="gridcell">
            <button
              className="button-view-task"
              aria-label={`View task "${task.title}"`}
              aria-description={`${task.numCompletedSubtasks} of ${task.numSubtasks} subtasks completed`}
              ...
              <span
                aria-hidden="true"
                className="text-task-subtasks">
                {`${task.numCompletedSubtasks} of ${task.numSubtasks}`} subtasks
              </span>
            </button>
          </div>
        </div>
